...
if(isset($update))
{
	...
	else
	{
		if($op==$opass)
		{
			...
			$err="<script>alert('Password Updated') </script>";
			}
			else
			{
			$err="<script>alert('Old Password and latest Password does not matched') </script>";
			}
		}
		else
		{
		$err="<script>alert('old password not matched') </script>";
		}			
	}
}
	?>
